...
function decode(buf, table, maxSize = 262144) {
	const headers = []
	let o = 0
	let listSize = 0
	// Bound the decoded header-list size. HPACK can expand a tiny block into huge output via
	// repeated indexed references to large dynamic-table entries (a decompression bomb), so cap
	// the running size at the MAX_HEADER_LIST_SIZE we advertise. Overrun throws a connection error.
	const push = (n, v) => {
		listSize += (n ? n.length : 0) + (v ? v.length : 0) + 32  // RFC 7541 §4.1 entry-size estimate
		if (listSize > maxSize) { const e = new Error('HPACK header list exceeds MAX_HEADER_LIST_SIZE'); e.code = 'EH2COMPRESS'; throw e }
		headers.push([n, v])
	}
	while (o < buf.length) {
		const b = buf[o]
