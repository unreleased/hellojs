...
const { resolveHttpsRecords } = require('./dns/https-records')
const { parseRetryConfigs, selectECHConfig } = require('./tls/ech-config')
...
const echBootstrapOverrides = new Map()
...
function selectEchContext(configList) {
	if (!configList) return null
	try {
		const config = selectECHConfig(parseRetryConfigs(configList))
		if (!config) return null
		return {
			config,
			publicName: config.publicName,
			retryConfigs: Buffer.from(configList),
		}
	} catch (_) {
		return null
	}
}
// Servers are allowed to securely replace a stale DNS-published ECHConfig with
// retry_configs. Cache that replacement per origin so later requests key the pool the
// same way the successful retry did instead of immediately reverting to the stale DNS view.

function rememberEchBootstrapOverride(host, bootstrap) {
	if (!host || !bootstrap?.ech?.config) return
	echBootstrapOverrides.set(String(host).toLowerCase(), {
		...bootstrap,
		ech: {
			...bootstrap.ech,
			retryConfigs: bootstrap.ech.retryConfigs ? Buffer.from(bootstrap.ech.retryConfigs) : null,
		},
	})
}

async function resolveRequestBootstrap(url, opts) {
	if (url.protocol !== 'https:') return null
	if (opts._echBootstrap) return opts._echBootstrap
	if (opts.ech !== true) return null
	const override = echBootstrapOverrides.get(url.hostname.toLowerCase())
	if (override) return override
	const bootstrap = await resolveHttpsRecords(url.hostname)
	if (!bootstrap) return null
	const ech = selectEchContext(bootstrap.echConfigList)
	if (bootstrap.svcbReliant && !ech) {
		throw new HellojsError('ECH bootstrap succeeded but no supported ECH config was usable', 'EPROTO')
	}
	return {
		...bootstrap,
		ech,
...
function buildSessionIdentity(host, tlsName, ech) {
	if (ech?.config) return { host, ech: true, publicName: ech.publicName || ech.config.publicName }
	if (tlsName && tlsName !== host) return { host, publicName: tlsName }
	return { host }
}

function buildEchRetryOpts(opts, bootstrap, err) {
	if (opts._echRetried || err?.code !== 'EECHREJECT' || !err.retryConfigs) return null
	const ech = selectEchContext(err.retryConfigs)
	if (!ech) return null
	return {
		...opts,
		_echRetried: true,
...
async function performRequest(opts) {
	...
	let bootstrapMemo = null
	let bootstrapLoaded = false
	const getBootstrap = async () => {
		if (!bootstrapLoaded) {
			bootstrapMemo = await resolveRequestBootstrap(url, opts)
			bootstrapLoaded = true
		}
		return bootstrapMemo
	}
	const bootstrap = opts.ech === true ? await getBootstrap() : null
	let transport = 'tcp'
	let actualPort = port
	if (opts.h3 === true) {
		transport = 'quic'
		actualPort = bootstrap?.port || port
	...
	const acquire = async (requestTransport, requestPort) => {
		const requestBootstrap = opts.ech === true ? bootstrap : null
		const ech = requestBootstrap?.ech || null
		const tlsName = host
		const connectHost = requestBootstrap?.connectName || host
		const requestAddressHints = requestBootstrap ? { v4: requestBootstrap.ipv4hint || [], v6: requestBootstrap.ipv6hint || [] } : null
		return defaultPool.acquire({
			host,
			port: requestTransport === 'tcp' ? (requestBootstrap?.port || requestPort) : requestPort,
			profile,
			proxy: opts.proxy,
			forceFresh: opts.forever === false || opts._echForceFresh === true,
			cacheConnection: opts.forever !== false,
			transport: requestTransport,
			earlyData: (opts.earlyData && requestTransport === 'tcp') ? Buffer.from(opts.earlyData) : null,
			verifyTLS: opts.verifyTLS !== false,
			timeouts: opts.timeouts,
			connectHost,
			tlsName,
			addressHints: requestTransport === 'tcp' || requestTransport === 'quic' ? requestAddressHints : null,
			ech: requestTransport === 'quic' || requestTransport === 'tcp' ? ech : null,
			sessionIdentity: buildSessionIdentity(host, tlsName, ech),
		})
	}

	let conn
	try {
		conn = await acquire(transport, actualPort)
	} catch (e) {
		// If h3 was attempted opportunistically and UDP failed, fall back to TCP.
		if (transport === 'quic' && opts.h3 !== true) {
			try {
				conn = await acquire('tcp', port)
			} catch (fallbackErr) {
				const retryOpts = buildEchRetryOpts(opts, bootstrap, fallbackErr)
				if (retryOpts) return performRequest(retryOpts)
				throw fallbackErr
			}
		} else {
			const retryOpts = buildEchRetryOpts(opts, bootstrap, e)
			if (retryOpts) return performRequest(retryOpts)
			throw e
		}
	}

	// A successful retry has now "healed" our local view of the origin, so persist the
	// replacement config for subsequent requests.

	if (opts._echRetried && bootstrap?.ech?.config) rememberEchBootstrapOverride(host, bootstrap)
	// `_echForceFresh` is only for the immediate retry handshake. Clear it before any
	// same-origin redirect follow-up so the refreshed pooled connection can be reused.

	if (opts._echForceFresh) delete opts._echForceFresh
	// Build headers
...
function handleResponse({ status, headers, body, raw, opts, resolve, reject }) {
	...
	if (opts.followRedirect !== false && status >= 300 && status < 400 && headers.location && (opts.maxRedirects ?? 10) > 0) {
		const redirectedUrl = new URL(headers.location, opts.url || opts.uri)
		const newOpts = { ...opts, url: redirectedUrl.toString(), maxRedirects: (opts.maxRedirects ?? 10) - 1 }
		if (status === 303) newOpts.method = 'GET'
		const prevOrigin = new URL(opts.url || opts.uri).origin
		if (redirectedUrl.origin !== prevOrigin) {
			delete newOpts._echBootstrap
			delete newOpts._echRetried
			delete newOpts._echForceFresh
		}
...
function request(opts, cb) {
	...
	// The callback API historically receives `(err, res, body)` even when the promise form
	// would have resolved to just `body`. Internally force full-response mode for callback
	// consumers, then project back to the old promise result when needed.
	const callbackNeedsFullResponse = !!cb && opts.resolveWithFullResponse !== true
	const runOpts = callbackNeedsFullResponse ? { ...opts, resolveWithFullResponse: true } : opts
	const fullResponsePromise = performWithRetry(runOpts)
	const returnPromise = callbackNeedsFullResponse ? fullResponsePromise.then((res) => res.body) : fullResponsePromise
	if (cb) {
		fullResponsePromise.then((res) => cb(null, res, res.body),
