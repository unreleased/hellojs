...
class TLS extends EventEmitter {
    ...
    this.appBuf   = Buffer.alloc(0)   // decrypted app bytes (HTTP/2)
		...
		// MLKEM state (set by connect() before socket open).
		this.mlkemPk = null
		this.mlkemSk = null
	...
	connect = async () => {
		// Pre-generate MLKEM-768 keypair (async). Needed before ClientHello can be sent.
	...
	startClientHello = (hrrCtx = null) => {
		...
		const cipherList = [(grease.a << 8) | grease.a, ...profileCiphers]
		...
		if (hrrCtx) {
			...
			const keyShare = CreateKeyShareX25519(grease.c, this.mlkemPk)
		...
		const EXT_BUILDERS = {
			...
			5:     () => CreateStatusRequestExtension(),                                                            // status_request
			10:    () => CreateSupportedGroupsExtension(grease.c, ptls.supportedGroups),                            // supported_groups
			...
			43:    () => CreateSupportedVersionsExtension(grease.b, ptls.supportedVersions),                         // supported_versions
		...
		// (we add our own boundary GREASEs), unknown IDs are skipped. Otherwise fall back to the
		// canonical Chrome 147 set so legacy profiles (e.g. chrome147-mac.js) keep working.
		...
		if (Array.isArray(ptls.extensionOrder) && ptls.extensionOrder.length) {
			...
			for (const e of ptls.extensionOrder) {
				...
				if (EXT_BUILDERS[e.id] != null) middleIds.push(e.id)
			}
		} else {
		...
		// Per-instance extension permutation — seeded by client_random for stability per instance.
		// First entry: GREASE_a; last entry: GREASE_b. Middle: shuffled middle.
		...
		const extPieces = [
			CreateGREASEExtension(grease.a),
			...middleNames.map(n => namedExts[n]),
			CreateGREASEExtension(grease.b),
		]
