...
const { buildECHOffer, confirmECHAcceptance } = require('./ech-clienthello')
...
class TLS extends EventEmitter {
	constructor(host, port, proxy, opts = {}) {
		...
		this.host = host
		// `connectHost` is the name we actually open the TCP / proxy CONNECT socket against.
		// It differs from `this.host` when HTTPS/SVCB bootstrap points the origin at a different
		// backend (alias / `target`), or when ECH routes us through a client-facing server.
		...
		// Session tickets are partitioned by logical TLS identity (plain host, public-name,
		// or ECH-mode). Without this, ECH and non-ECH tickets for the same `host` would
		// stomp each other and confuse server-side resumption decisions.
		this.sessionIdentity = opts.sessionIdentity || host
		// Optional pre-resolved address hints from the HTTPS RR bootstrap. Forwarded straight
		// to happy-eyeballs so the connect race doesn't redo work the bootstrap already did.
		...
		// Real ECH offer state. `this.ech` is the selected ECHConfig + retry_configs context
		// from the client layer; `echOffered` is the in-flight offer (set in startClientHello,
		// consulted in ServerHello to confirm acceptance and swap the transcript).
	...
	// Conservative ECH termination. The current TCP path commits to one real ECH offer per
	// connection: HRR reseal and post-rejection retry/public-name fallback live at the
	// client layer (see `buildEchRetryOpts`). This helper centralizes the abort + error
	// shape so callers in the ServerHello path can surface EECHREJECT (with retry_configs)
	// vs EECHUNSUPPORTED uniformly.
	_abortECH(message, extra = {}) {
		const err = new Error(`ECH offer path is not fully wired yet: ${message}`)
		...
		if (this.proxy) {
			...
			if (proxyUrl.protocol === 'socks5:' || proxyUrl.protocol === 'socks5h:') {
				...
				socks5Connect({
					...
					this.startClientHello().catch((err) => {
						this.emit('error', err)
						this.socket?.destroy(err)
					})
			...
			this.socket = net.createConnection(proxyOptions, () => {
				...
				let connectHeaders = `CONNECT ${this.connectHost}:${this.port} HTTP/1.1\r\nHost: ${this.connectHost}:${this.port}\r\n`;
			...
			happyConnect({ host: this.connectHost, port: this.port, connectTimeoutMs: 10_000, hints: this.addressHints })
		...
		while (this.encHsBuf.length >= 4) {
			...
			switch (msgType) {
				...
				case MESSAGE_TYPES.SERVER_CERTIFICATE:
					this.transcript.push(full)
					// Parse + (optionally) validate the cert chain. TLS 1.3 Certificate wire format
					// (RFC 8446 §4.4.2): 1 byte cert_request_context length + (empty for server) +
					// 3-byte CertificateList length + entries (3-byte cert length + DER + 2-byte
					...
					try {
						...
						while (p < end) {
							...
							p += clen
							const eLen = (body[p] << 8) | body[p+1]; p += 2 + eLen   // skip cert extensions
						}
						...
						if (this.verifyTLS) {
							...
							if (!r.ok) {
								const e = new Error(`certificate validation failed: ${r.reason}`)
								e.code = 'ECERTVALIDATION'
								this.emit('error', e)
								this.socket?.destroy()
								return
							}
						...
						const err = new Error(`Certificate parse failed: ${e.message}`)
						err.code = 'ECERTPARSE'
						this.emit('error', err)
						this.socket?.destroy()
						return
					...
					break;
				case 0x19: // CompressedCertificate (RFC 8879). Server may send this in place of Certificate when client advertised compress_certificate. The wire bytes themselves go into the transcript hash; we don't need to decompress unless we're validating the cert.
					this.transcript.push(full)
					break;
				...
				case 0x04: { // NewSessionTicket — post-handshake. Parse + cache the ticket
					...
					try {
						...
						sessionCache.put(this.sessionIdentity, session)
    ...
    while (o + 4 <= end) {
      ...
      if (extType === 0x002b) { // supported_versions (ServerHello / HRR)
        ...
        hrrCookie = extData.subarray(2, 2 + cklen)
			...
			echExtensionData = Buffer.from(extData)
		...
		if (isHRR) {
			if (this.echOffered) {
				const accepted = confirmECHAcceptance({
					...
					serverHello: fullMsgBytes,
					hashName: this.cipher.hash,
					helloRetryRequest: true,
				})
				this.echOffered.accepted = accepted
				return this._abortECH(accepted
					? 'server accepted ECH in HelloRetryRequest, but CH2 resealing is not wired yet'
					: 'server rejected ECH in HelloRetryRequest, and retry handling is not wired yet', {
						code: echExtensionData ? 'EECHREJECT' : 'EECHUNSUPPORTED',
						retryConfigs: echExtensionData && echExtensionData.length !== 8 ? echExtensionData : null,
					})
			}
    ...
    this.serverEMS = serverEMS
		...
		if (this.echOffered) {
			const accepted = this.server.selVersion === 0x0304 && confirmECHAcceptance({
				clientHelloInner: this.echOffered.innerClientHelloHandshake,
				serverHello: fullMsgBytes,
				hashName: this.cipher.hash,
			})
			this.echOffered.accepted = accepted
			if (!accepted) {
				return this._abortECH('server rejected ECH, and retry/public-name handling is not wired yet', {
					code: echExtensionData ? 'EECHREJECT' : 'EECHUNSUPPORTED',
	...
	_handleHelloRetryRequest(selectedGroup, cookieBytes) {
		...
		this.startClientHello({ keyShareExt, cookieExt }).catch((err) => {
			this.emit('error', err)
			this.socket?.destroy(err)
		})
	}
	...
	// async because the real-ECH path awaits HPKE seal() inside buildECHOffer(). The
	// non-ECH path stays synchronous in spirit (no awaits), but the function signature
	// has to be `async` to host the ECH branch.
	startClientHello = async (hrrCtx = null) => {
		...
		// Real ECH requires a fresh KEM + key_share offer; reusing a PSK on the same connection
		// would force us to maintain two parallel transcripts and is not currently wired up.
		const realEchRequested = !hrrCtx && !!this.ech?.config
		if (realEchRequested && this.session) {
			throw new Error('tls: real ECH with session resumption is not implemented yet')
		}
		const canOfferRealECH = realEchRequested
		const echConfig = canOfferRealECH ? this.ech.config : null
		// Outer SNI must advertise the client-facing public name from the ECHConfig — the
		// inner ClientHello carries the true `this.host` SNI under HPKE.
		const outerServerName = canOfferRealECH ? (this.ech.publicName || echConfig.publicName) : this.host
		...
		const EXT_BUILDERS = {
			0:     () => CreateSNIExtension(outerServerName),                                                      // server_name
			...
			65037: () => canOfferRealECH ? Buffer.alloc(0) : CreateEncryptedClientHelloExtension(),                  // ECH GREASE / real ECH placeholder
		...
		let handshakeMsg
		if (canOfferRealECH) {
			const advertisedVersions = Array.isArray(ptls.supportedVersions) && ptls.supportedVersions.length
				? ptls.supportedVersions
				: [0x0304, 0x0303]
			if (!advertisedVersions.includes(0x0304)) throw new Error('tls: real ECH requires a TLS 1.3-capable profile')
			if (!keyShareExtension) throw new Error('tls: real ECH requires key_share support in the active profile')
			if (!middleNames.includes('65037')) throw new Error('tls: profile has no encrypted_client_hello slot for a real ECH offer')
			const outerExtensions = []
			const innerExtensions = []
			let echExtensionIndex = -1
			const innerSupportedVersions = Array.isArray(ptls.supportedVersions) && ptls.supportedVersions.length
				? ptls.supportedVersions.filter((version) => version >= 0x0304)
				: [0x0304]
			if (!innerSupportedVersions.length) innerSupportedVersions.push(0x0304)
			if (useGrease) {
				const leadingGrease = CreateGREASEExtension(grease.a)
				outerExtensions.push(leadingGrease)
				innerExtensions.push(leadingGrease)
			}
			for (const n of middleNames) {
				if (n === '65037') {
					echExtensionIndex = outerExtensions.length
					continue
				}
				const outerExt = namedExts[n]
				if (!Buffer.isBuffer(outerExt)) continue
				outerExtensions.push(outerExt)
				if (n === '0') {
					innerExtensions.push(CreateSNIExtension(this.host))
				} else if (n === '43') {
					innerExtensions.push(CreateSupportedVersionsExtension(grease.b, innerSupportedVersions, { useGrease }))
				} else {
					innerExtensions.push(outerExt)
				}
			}
			if (useGrease) {
				const trailingGreaseValue = grease.b === grease.a ? (((grease.b + 0x1010) & 0xffff) || 0x0a0a) : grease.b
				const trailingGrease = CreateGREASEExtension(trailingGreaseValue)
				outerExtensions.push(trailingGrease)
				innerExtensions.push(trailingGrease)
			}
			...
			for (const n of middleNames) {
				const ext = namedExts[n]
				if (ext != null) extPieces.push(ext)
			}
			...
			hsLen.writeUIntBE(clientHelloBody.length, 0, 3)
