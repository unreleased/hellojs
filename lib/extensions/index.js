...
// Two-mode ECH extension builder.
//   - Called with no arguments → GREASE-shaped extension with random bytes. This is the
//     legacy code path used when no published ECHConfig is being offered, so we still emit
//     a credible "I support ECH" hint without leaking inner ClientHello data.
//   - Called with { type, cipherSuite, configId, enc, payload } → a real RFC 9849
//     encrypted_client_hello extension. The outer ClientHello uses type='outer' and the
//     inner ClientHello uses type='inner' (single-byte sentinel).
const CreateEncryptedClientHelloExtension = (options = null) => {
	const ech = Buffer.from([0xfe, 0x0d])
	if (!options) {
		const enc = crypto.randomBytes(32)
		const payload = crypto.randomBytes(240)
		return CreateEncryptedClientHelloExtension({
			type: 'outer',
			cipherSuite: { kdfId: 0x0001, aeadId: 0x0001 },
			configId: 0xe9,
			enc,
			payload,
		})
	...
	if (type === 'inner') {
		return Buffer.from([0xfe, 0x0d, 0x00, 0x01, 0x01])
	}
	if (type !== 'outer') throw new Error(`unsupported ECH extension type ${type}`)
	const cipherSuite = options.cipherSuite || {}
	const kdfId = cipherSuite.kdfId
	const aeadId = cipherSuite.aeadId
	const configId = options.configId
	const enc = options.enc || Buffer.alloc(0)
	const payload = options.payload
	if (!Number.isInteger(kdfId) || kdfId < 0 || kdfId > 0xffff) throw new TypeError('ECH cipherSuite.kdfId must be a uint16')
	if (!Number.isInteger(aeadId) || aeadId < 0 || aeadId > 0xffff) throw new TypeError('ECH cipherSuite.aeadId must be a uint16')
	if (!Number.isInteger(configId) || configId < 0 || configId > 0xff) throw new TypeError('ECH configId must be a uint8')
	if (!Buffer.isBuffer(enc)) throw new TypeError('ECH enc must be a Buffer')
	if (!Buffer.isBuffer(payload) || payload.length === 0) throw new TypeError('ECH payload must be a non-empty Buffer')
	const encLength = Buffer.alloc(2)
	encLength.writeUInt16BE(enc.length, 0)
	const payloadLength = Buffer.alloc(2)
	payloadLength.writeUInt16BE(payload.length, 0)
	const echBody = Buffer.concat([
		Buffer.from([0x00]),
		Buffer.from([(kdfId >> 8) & 0xff, kdfId & 0xff, (aeadId >> 8) & 0xff, aeadId & 0xff]),
	...
	echLength.writeUInt16BE(echBody.length, 0)
	return Buffer.concat([ech, echLength, echBody])
}
