...
class Pool {
	constructor(opts = {}) {
		...
		// host:port -> { count, waiters: [] }. Cap on simultaneous fresh-handshake / forever:false
		// connections. The pooled keep-alive connection doesn't count against this — only extra
		// connections created when forceFresh is set or when the pool entry is in use.
		this.hostState = new Map()
	}

	keyOf(host, port, profile, proxy, transport) {
		return `${transport || 'tcp'}|${host}:${port}|${profile || 'default'}|${proxy || 'noproxy'}`
	}
	...
	acquire({ host, port = 443, profile, proxy, forceFresh = false, transport = 'tcp', earlyData = null, verifyTLS = true, timeouts = null, profileObj = null }) {
		if (this._draining) {
			const { HellojsError } = require('./errors')
			return Promise.reject(new HellojsError('pool is draining; no new requests accepted', 'EPROTO'))
		}
		const key = this.keyOf(host, port, profile, proxy, transport)
		...
		// Synchronously install the pending promise BEFORE awaiting the slot — so the next
		// concurrent caller sees it on the fast path above.
		const p = this._acquireSlot(host, port).then(async () => {
			let conn
			try {
				conn = transport === 'quic'
					? await this._createH3(host, port, profile, key)
					: await this._create(host, port, profile, proxy, key, { earlyData, verifyTLS, timeouts, profileObj: profileObj || profile })
			} catch (e) {
				this._releaseSlot(host, port)
				throw e
			}
			if (!forceFresh) this.connections.set(key, conn)
	...
	_createH3(host, port, profile, key) {
		return new Promise((resolve, reject) => {
			const qc = new QuicConnection(host, port)
			const conn = new PooledConnection(null, key)
			conn.quicConn = qc
			let settled = false
			qc.on('ready', () => {
				if (settled) return
	...
	_create(host, port, profile, proxy, key, opts = {}) {
		return new Promise((resolve, reject) => {
			...
			const session = sessionCache.take(host)
			const tlsOpts = {}
			if (session) tlsOpts.session = session
			if (opts.earlyData) tlsOpts.earlyData = opts.earlyData
			tlsOpts.verifyTLS = opts.verifyTLS !== false
			if (opts.profileObj) tlsOpts.profile = opts.profileObj
			const tls = new TLS(host, port, proxy, tlsOpts)
			const conn = new PooledConnection(tls, key)
			...
			const onReady = (alpn) => {
				...
				if (alpn === 'h2' && conn.h2Session) {
					...
					// h1 has no session object. Evict the pooled connection when the socket closes
					// (e.g. the server's keep-alive idle timeout) so we never hand back a dead
					// connection for reuse — the h1 path keeps the socket warm between requests.
					tls.socket?.once?.('close', () => this._evict(key, conn))
				}
				resolve(conn)
			}
