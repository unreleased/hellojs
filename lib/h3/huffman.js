...
//
// We use the canonical table from the RFC. Decoding walks the bit stream and emits
// a symbol when a complete code is matched. This implementation builds a simple
// lookup table once (256 entries × 2 directions of branching) — straightforward
// 8-bit DFA approach.

// Code table: [symbol]: [code (hex), bit_length]. EOS = 256 with 30-bit code 0x3fffffff.
const TABLE = (() => {
	...
	for (let i = 0; i < data.length; i++) t[i] = data[i]
	return t
})()

// Build a flat lookup map: codeBits (left-aligned in MSB) → symbol.
// Decode is bit-by-bit accumulation into a buffer; on each bit, check all entries with that prefix length.
// More efficient: build a 1-bit tree.
let _tree = null
function buildTree() {
	const tree = { children: [null, null] }
	for (let sym = 0; sym < 256; sym++) {
		const [code, len] = TABLE[sym]
		let node = tree
...
function decode(buf) {
	...
	const out = []
	let node = _tree
	let bitsTotal = buf.length * 8
	for (let i = 0; i < bitsTotal; i++) {
		const byte = buf[i >> 3]
		const bit = (byte >> (7 - (i & 7))) & 1
		node = node.children[bit]
		if (!node) throw new Error('huffman: invalid sequence')
		if (node.symbol !== undefined) {
			out.push(node.symbol)
			node = _tree
		}
	}
	// Remaining padding (1-bits) is discarded; tree at root or partial path means OK.
	return Buffer.from(out)
}

// Encode a string/buffer to Huffman-coded bytes per RFC 7541 Appendix B.
// Pads the final byte with 1-bits (EOS prefix) so the decoder discards them safely.
// Uses BigInt for the bit accumulator — RFC codes are up to 30 bits, and JS bitwise
// operators are 32-bit which is too narrow when paired with a residual high-bit carry.
function encode(input) {
	...
	for (const byte of bytes) {
		const [code, len] = TABLE[byte]
		buf = (buf << BigInt(len)) | BigInt(code)
		bits += len
		while (bits >= 8) {
			bits -= 8
			out.push(Number((buf >> BigInt(bits)) & 0xffn))
			buf = buf & ((1n << BigInt(bits)) - 1n)
		}
	}
	if (bits > 0) {
		const pad = (1 << (8 - bits)) - 1
		out.push(Number(((buf << BigInt(8 - bits)) | BigInt(pad)) & 0xffn))
	}
	return Buffer.from(out)
}

module.exports = { decode, encode, TABLE }
