...
async function resolveBoth(host) {
	...
	const v6 = dns.promises.resolve6(host).catch(() => [])
	const v4 = dns.promises.resolve4(host).catch(() => [])
...
async function happyConnect({ host, port, connectTimeoutMs = 5000 }) {
	const { v6, v4 } = await resolveBoth(host)
