...
function buildH2Headers({ method, host, path, userHeaders, profile }) {
	...
	if (userHeaders) {
		for (const [k, v] of Object.entries(userHeaders)) {
			if (k.toLowerCase() === 'host') continue  // folded into :authority above
			h[k.toLowerCase()] = v
		}
	}
	return h
...
function buildH1Headers({ host, userHeaders, profile }) {
	...
	// Host is builder-managed: emit exactly one, first (Chrome's h1 ordering). A caller-supplied
	// Host header overrides the URL-derived value rather than producing a duplicate Host line
	// (two Host headers are malformed per RFC 7230 5.4 and a fingerprint tell).
	const hostValue = merged.host != null ? merged.host : host
	delete merged.host
